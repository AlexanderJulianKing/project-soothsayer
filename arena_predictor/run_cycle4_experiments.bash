#!/usr/bin/env bash
# Cycle 4: Post-imputation pipeline experiments (ALT pathway, features, ensembling)
...
# Helper: run experiment, extract RMSE from its isolated output dir
run_exp() {
    ...
    python3 -u predict.py $BASE_FLAGS $CV_FLAGS $PARALLEL_FLAGS \
        --output_root "$out_root" $flags > "/tmp/cycle4_exp${exp_id}.log" 2>&1
    local rmse
...
echo "=== Cycle 4: Post-imputation pipeline experiments ==="
echo "Started: $(date)"

# Batch 1: Exp 0, 1, 2 in parallel
echo ""
echo "--- Batch 1: Exp 0 (Baseline), 1 (ALT bagged), 2 (Delta head) ---"
run_exp 0 "Baseline"             ""                    &
run_exp 1 "ALT bagged 5 seeds"  "--alt_bag_seeds 5"   &
run_exp 2 "Delta head"          "--delta_head"         &
wait
echo "Batch 1 done: $(date)"

# Batch 2: Exp 3, 4, 5 in parallel
echo ""
echo "--- Batch 2: Exp 3 (Reliability gate), 4 (Imputer ensemble), 5 (Obs-weighted SVD) ---"
run_exp 3 "ALT reliability gate" "--alt_reliability_gate" &
run_exp 4 "Imputer ensemble 3s"  "--imputer_seeds 3"     &
run_exp 5 "Obs-weighted SVD"     "--obs_weighted_svd"     &
wait
echo "Batch 2 done: $(date)"

# Batch 3: Exp 6 alone
echo ""
echo "--- Batch 3: Exp 6 (ALT LOO calibrate) ---"
run_exp 6 "ALT LOO calibrate"   "--alt_loo_calibrate"
echo "Batch 3 done: $(date)"

# Collect results
echo ""
echo "=== CYCLE 4 RESULTS ==="
for i in 0 1 2 3 4 5 6; do
    grep "^Exp $i " /tmp/cycle4_exp${i}.log 2>/dev/null || \
    python3 -c "
import json,glob
